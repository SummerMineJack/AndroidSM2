...

{
	...
	public void sm2Sign(byte[] md, BigInteger userD, ECPoint userKey, SM2Result sm2Result) {
		...
		do {
			do {
				 ...
				 * this.ecc_point_g.multiply(k);
				 */
				// LOG.debug("计算曲线点X1:{} " ,
				// kp.getX().toBigInteger().toString(16));
				// LOG.debug("计算曲线点Y1: {} " ,
				// kp.getY().toBigInteger().toString(16));
	...
	public void sm2Verify(byte md[], ECPoint userKey, BigInteger r, BigInteger s, SM2Result sm2Result) {
		...
		if (t.equals(BigInteger.ZERO)) {
			...
			// LOG.debug("计算曲线点X0: {} ",
			// x1y1.getX().toBigInteger().toString(16));
			// LOG.debug("计算曲线点Y0: {} ",
			// x1y1.getY().toBigInteger().toString(16));
